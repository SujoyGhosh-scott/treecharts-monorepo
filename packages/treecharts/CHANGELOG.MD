...
- **Right-Angle Bottom-to-Top Connections**: Fixed missing connections in the final level of right-angle trees when using bottom-to-top alignment. Completely rewrote RightAngleRenderer's connection logic using a simpler, more reliable child-first iteration approach and enhanced ConnectionDrawer with high-level methods for better architecture and text positioning.
...
- **Shadow Rendering Issues**: Resolved critical visual corruption when shadows were enabled, fixing duplicate shadow elements and opacity issues that created overlapping black shapes
- **Node Duplication**: Eliminated triple rendering layers causing nodes to appear multiple times by streamlining the rendering pipeline between NodeDrawer and renderer classes
- **Gradient Rendering**: Fixed gradient styling not being applied to rectangle nodes where solid colors were overwriting gradient fills
- **Custom Shape Support**: Fixed custom SVG path shapes not rendering correctly at proper positions, resolving missing customAttributes parameter passing and coordinate positioning
- **Right-Angle Arrow Positioning**: Fixed arrows appearing on incorrect segments in right-angle connections, ensuring "source-to-target" and "target-to-source" arrows appear on the correct connection segments
- **Right-Angle Arrow Directions**: Fixed "both" arrow direction showing arrows on both ends of every vertical edge instead of correctly placing one arrow pointing to parent and one pointing to child
- **EdgeConfig Cleanup**: Removed unused `type` property from EdgeConfig interface that was duplicating the main TreeChart type configuration, simplifying the API
- **ConnectionDrawer Optimization**: Cleaned up unused method parameters and improved curve rendering with proper curveRadius utilization
- **Bottom-to-Top Alignment**: Fixed critical tree alignment bug where `horizontalAlign: "bottom-to-top"` incorrectly showed root at top instead of bottom by implementing proper tree level reversal
- **All-Direction Alignment**: Fixed all-direction charts becoming invisible when horizontal alignment options were applied, since alignment concepts don't apply to radial layouts
...
- **NodeDrawer Code Optimization**: Major refactoring to eliminate code duplication and improve maintainability
  - **50% Code Reduction**: Simplified `calculateDynamicNodeWidth` and `calculateDynamicNodeHeight` methods from 120+ lines to 60 lines
  - **Extracted Utility Methods**: Created reusable `createTextMeasurementContext()`, `measureText()`, `calculateTextWrapping()`, and `delegateCalculation()` methods
  - **Eliminated Duplication**: Removed 4+ instances of duplicate canvas creation and text measurement logic
  - **Improved Error Handling**: Added proper fallback mechanisms for text measurement failures
  - **Enhanced Maintainability**: Centralized common operations into single-responsibility utility methods
